...
export async function GET() {
  try {
    const res = await fetch(resolveGatewayUrl("/admin/kitchen/peak-mode"), {
    ...
    return NextResponse.json({ error: exc?.message ?? "peak mode fetch failed" }, { status: 503 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({ error: exc?.message ?? "peak mode update failed" }, { status: 503 });
  }
}
