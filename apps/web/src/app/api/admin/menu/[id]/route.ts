...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const res = await fetch(resolveGatewayUrl(`/admin/menu/${id}`), {
      method: "PUT",
    ...
    return NextResponse.json({ error: exc?.message ?? "menu update failed" }, { status: 503 });
  }
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: exc?.message ?? "menu patch failed" }, { status: 503 });
  }
}
