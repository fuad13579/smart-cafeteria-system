...
export async function GET() {
  try {
    const res = await fetch(resolveGatewayUrl("/admin/menu"), {
    ...
    return NextResponse.json({ error: exc?.message ?? "menu fetch failed" }, { status: 503 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: exc?.message ?? "menu create failed" }, { status: 503 });
  }
}
