...
export async function POST(
  req: Request,
  ctx: { params: Promise<{ main: string; slot: string }> }
) {
  const { main, slot } = await ctx.params;
  try {
    const body = await req.json().catch(() => ({}));
    const res = await fetch(resolveGatewayUrl(`/admin/menu/slots/${main}/${slot}/items`), {
      method: "POST",
    ...
    return NextResponse.json({ error: exc?.message ?? "menu slot assignment failed" }, { status: 503 });
  }
}
