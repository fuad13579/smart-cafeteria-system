...
export async function GET() {
  try {
    const res = await fetch(resolveGatewayUrl("/admin/menu/visibility"), {
    ...
    return NextResponse.json({ error: exc?.message ?? "visibility fetch failed" }, { status: 503 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({ error: exc?.message ?? "visibility update failed" }, { status: 503 });
  }
}
