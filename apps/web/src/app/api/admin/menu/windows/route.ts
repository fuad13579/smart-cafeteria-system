...
export async function GET() {
  try {
    const res = await fetch(resolveGatewayUrl("/admin/menu/windows"), {
    ...
    return NextResponse.json({ error: exc?.message ?? "windows fetch failed" }, { status: 503 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: exc?.message ?? "window create failed" }, { status: 503 });
  }
}
