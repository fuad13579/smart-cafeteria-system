...
export function MobileTabBar({ cartCount }: { cartCount: number }) {
  ...
  const Tab = ({ href, label }: { href: string; label: string }) => {
    ...
    return (
      <Link
        href={href}
        className={[
          "flex flex-1 flex-col items-center justify-center gap-1 py-1.5 text-xs",
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-zinc-200 bg-white/90 backdrop-blur dark:border-zinc-900 dark:bg-zinc-950/90 sm:hidden">
